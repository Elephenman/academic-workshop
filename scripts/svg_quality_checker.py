...
class SVGQualityChecker:
    ...
    def check_directory(self, directory: str, expected_format: str = None) -> List[Dict]:
        ...
        # YELLOW-05: spec_lock page count consistency check
        self._check_spec_lock_page_count(dir_path, svg_files)

        return self.results

    def _check_spec_lock_page_count(self, dir_path: Path, svg_files: list) -> None:
        """YELLOW-05: Check SVG file count matches spec_lock.md page count.

        If spec_lock.md declares N pages but we found M SVG files,
        emit a warning when counts differ.
        """
        if _parse_spec_lock is None:
            return

        # Look for spec_lock.md
        for candidate in (dir_path / 'spec_lock.md',
                          dir_path.parent / 'spec_lock.md'):
            if candidate.exists():
                try:
                    lock_data = _parse_spec_lock(candidate)
                except Exception:
                    return

                # Extract page count from rhythm map
                rhythm = lock_data.get('rhythm', {})
                if not rhythm:
                    return

                expected_pages = len(rhythm)
                actual_pages = len(svg_files)

                if actual_pages != expected_pages:
                    print(f"\n[WARN] spec_lock page count mismatch: "
                          f"spec_lock declares {expected_pages} pages, "
                          f"but found {actual_pages} SVG files.")
                    print(f"       spec_lock: {candidate}")
                    print(f"       Tip: Update spec_lock.md or add/remove SVG files to match.\n")
                else:
                    print(f"[OK] spec_lock page count: {expected_pages} pages = {actual_pages} SVG files")
                return
        # No spec_lock.md found - silently skip
