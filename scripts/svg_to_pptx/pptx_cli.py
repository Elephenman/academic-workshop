...
''',
    ...
    parser.add_argument('--force-compat', action='store_true',
                        help='Force PNG compat mode even on Windows (requires svglib + CJK fonts)')
    ...
    # Windows compat mode: svglib FT_Exception on Windows with CJK fonts
    # Auto-disable PNG fallback rendering on Windows unless --force-compat
    use_compat = not args.no_compat
    if use_compat and sys.platform == 'win32' and not getattr(args, 'force_compat', False):
        use_compat = False
        if verbose:
            print("[INFO] Windows detected: PNG compat mode auto-disabled (svglib CJK font issue).")
            print("       Use --force-compat to override (requires cairosvg).")
