...
def parse_total_md(
    ...
    # YELLOW-06: Replace date placeholders with current date
    today_cn = datetime.now().strftime('%Y年%m月%d日')
    today_en = datetime.now().strftime('%Y-%m-%d')
    content = content.replace('{{DATE}}', today_cn)
    content = content.replace('{{DATE_CN}}', today_cn)
    content = content.replace('{{DATE_EN}}', today_en)
    # Also replace isolated YYYY年MM月DD日 patterns that look like placeholders
    # (e.g. "2026年01月01日" where month/day are 01)
