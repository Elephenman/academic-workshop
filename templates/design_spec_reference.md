...
## XII. Academic Workshop Extensions

> 以下为学术工坊在ppt-master原始模板基础上新增的2个确认项和学术场景默认推荐值。

### VIII-A. Figure Usage Strategy (Academic Item #8)

> ⭐ 学术工坊新增确认项。组会PPT几乎必定引用论文Figure。

| Strategy | When to Use | SVG Reference |
|----------|-------------|---------------|
| **Embed original figure** | Key result pages (breathing rhythm) | `<image href="images/figure_X.png" .../>` |
| **Path reference only** | Supplementary results | Mention in notes, not in SVG |
| **SVG flow diagram** | Method/process pages | Draw with rect+line+text+arrowhead |
| **Honest placeholder** | Figure not yet available | Gray rect + "Figure X: [caption]" text |

**Forbidden**:
- AI-generated images replacing original figures
- CSS silhouettes / SVG hand-drawn substitutes for real images
- Decorative images with no intrinsic connection to content

### IX-A. Academic Page Type Rhythm

> Default page rhythm mapping for academic papers.

| Page Type | Rhythm Tag | Design Rules | SVG Naming |
|-----------|-----------|--------------|------------|
| Cover | `anchor` | Title + Authors + Journal + Date; no decorative gradient bg | P01_cover.svg |
| Background | `dense` | Text-focused, max 3 key points; citations use [1] not emoji | P02_background.svg |
| Method/Process | `dense` | Flowchart/step bars, no fancy animation | P03_method.svg |
| Key Results | `breathing` ⭐ | Large Figure + one-sentence interpretation | P04_results.svg |
| Discussion | `dense` | Lists or tables, concise | P05_discussion.svg |
| Summary/Outlook | `anchor` | 1-3 core conclusions + 1 future direction, large whitespace | P06_summary.svg |
| Acknowledgments | `anchor` | Simple thanks, no over-decoration | P07_thanks.svg |

### Academic Default Color Palette

| Role | HEX | Purpose |
|------|-----|---------|
| Background | `#FFFFFF` | Content page background |
| Warm background | `#F8F8F6` | Alternative warm white |
| Primary | `#003366` | ZJU Blue / academic blue |
| Primary deep | `#001A33` | Deep academic blue |
| Accent | `#1A3A5C` | Data highlights |
| Body text | `#1D1D1F` | Main body text |
| Secondary text | `#545458` | Subtitles, secondary text |
| Tertiary text | `#86868B` | Annotations, source citations |
| Border/divider | `#E5E5EA` | Card borders, dividers |
| Card background | `#F8F8F6` | Card containers |

### Academic Default Font Stacks

| Role | Font Stack |
|------|-----------|
| Title (Latin-led) | `Georgia, "Microsoft YaHei", serif` |
...
- Body font size: **18px** (projection minimum)
- All sizes derived as ratios per §IV
